...
import ru.job4j.refactoring.repository.UserRepository;
import ru.job4j.refactoring.test.UserFakeRepository;
...
public class TgRemoteService extends TelegramLongPollingBot {
    ...
    /*Этот метода вызывается каждый раз, когда бот получает обновление (например, новое сообщение)*/
  /*  @Override
    public void onUpdateReceived(Update update) {
        if (update.hasMessage() && update.getMessage().hasText()) { *//*Проверяется, содержит ли обновление сообщение и текст. Если да, продолжается дальнейшая обработка*//*
            String messageText = update.getMessage().getText(); *//*Здесь извлекается текст, который отправил пользователь,*//*
            long chatId = update.getMessage().getChatId();  *//*и идентификатор чата, чтобы бот знал, куда отправлять ответ*//*
            *//*Создаётся объект SendMessage, который будет отправлен обратно пользователю.
            В поле text задаётся сообщение, содержащее текст: "Вы написали: [сообщение пользователя]"*//*
            SendMessage message = new SendMessage();
            message.setChatId(chatId);
            message.setText("Вы написали: " + messageText);
            *//*Метод execute() отправляет сообщение через Telegram API.
            Если во время отправки произойдёт ошибка, она будет обработана в блоке catch, и исключение будет выведено в консоль*//*
            try {
                execute(message);
